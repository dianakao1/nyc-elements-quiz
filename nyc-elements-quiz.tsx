...
/* ---------------- DESIGN TOKENS ---------------- */
const INK = "#17161A", PAPER = "#F3F2EE", MUTED = "#615D56", HAIR = "rgba(23,22,26,0.12)";
// WCAG AA text variants of each element color (>=4.5:1 on white and on jewel panels)
const TEXT_SAFE = { light: "#7A6300", tide: "#0B6E3E", ember: "#B04510", storm: "#1D50A2", echo: "#0B6570", mist: "#54687A", iron: "#4E5156", bloom: "#3B7326", shadow: "#3E3A47", dream: "#9C2B91", frost: "#3F7290", rock: "#B7271D" };
...
".acc-outer { display: grid; transition: grid-template-rows 0.34s cubic-bezier(0.22, 0.72, 0.24, 1), visibility 0.34s; }",
".acc-panel { transition: opacity 0.26s ease 0.05s, transform 0.32s cubic-bezier(0.22, 0.72, 0.24, 1); }",
"@media (prefers-reduced-motion: reduce) { .anim-up, .anim-pop, .anim-draw { animation: none !important; } .q-answer, .q-btn { transition: none !important; } .deck-inner, .deck-top .deck-inner { animation: none !important; } .deck-card { transition: none !important; } .acc-outer, .acc-panel { transition: none !important; } }",
...
const ELEMENTS = {
  ...
  dream: {
    name: "Dream", color: "#B933AD", dark: false, numeral: "IX",
  ...
  echo: {
    name: "Echo", color: "#0E7C86", dark: false, numeral: "XI",
  ...
  mist: {
    ...
    hero: { emoji: "🪼", name: "Wisp the Jellyfish",
      bio: "A moon jelly who drifts up the East River when the summer blooms roll in, a real, gently weird New York phenomenon. Wisp keeps her own time, the way fog does: everywhere at once, arriving exactly when she means to.",
    ...
    wildcard: { name: "Early-morning Staten Island Ferry (free)", where: "The Harbor", note: "Ride it at dawn in the fog, coffee in hand, even when it's brick out, and come straight back. Wisp calls it 'commuting to nowhere, beautifully.'" },
  },
};
...
/* ---------------- PROFESSIONS ---------------- */
const PROFESSIONS = {
  finance: { name: "Momentum Steward", field: "finance",
    sig: { desc: "An instinct for flows, of money, energy, and attention, and for when to hold steady versus when to let a day accelerate. In daily life it often looks like calm sequencing rather than urgency: you booked the table, watched the clock without anyone feeling watched, and knew when to call the night. Its quiet strength is stewardship; time, energy, and care arrive where they matter because you noticed early. It surfaces most when stakes and logistics stack up at once, and others feel it as steadiness. Its healthy counterweight is deliberate drift: hours budgeted for nothing, which this reading deals you on purpose.", ev: ["Timing & spend-posture answers", "First-instinct city questions", "Conviction-weighted vote pattern"], tags: ["strategic-thinking", "planning", "decision-making", "execution", "pattern-recognition", "long-term-orientation"] }, blurb: "You have a feel for when to hold steady and when to let life accelerate. Calm is your native tempo; timing is your love language.",
    picks: [
  ...
  engineering: { name: "Systems Harmonizer", field: "engineering",
    sig: { desc: "An eye for the structure beneath the surface: the load paths in a plan, the joints in a routine. Day to day it can look like quietly fixing the wobble everyone else routes around, and learning fastest by taking the thing apart, literally or mentally. Its strength is dependable elegance, solutions that keep working after you leave the room. It emerges when something needs to bear weight: a schedule, a system, a friendship under strain. Its balance is honoring what works without needing to know why, at least occasionally.", ev: ["Structure & mechanism answers", "View and route preferences", "Repeat strong votes for how-it-works reads"], tags: ["systems-thinking", "problem-solving", "precision", "curiosity", "execution", "continuous-improvement"] }, blurb: "You see how things hold together, and you can't help but true the wobble. Understanding, for you, is a form of affection.",
    picks: [
  ...
  realestate: { name: "Horizon Builder", field: "real estate",
    sig: { desc: "A knack for reading places and moments for what they could become rather than only what they are. It often makes you an early spotter of potential, in a block, an idea, a person, and a patient assembler once you have spotted it, comfortable with returns that take seasons to arrive. Its strength is durable optimism: hope with a foundation poured under it. It surfaces in decisions about where to plant time and energy. Its balancing tendency is presence, letting a finished thing be finished and a good view just be a view.", ev: ["Long-horizon & view answers", "Neighborhood-instinct reads", "Spend posture across the quiz"], tags: ["vision", "planning", "opportunity-recognition", "patience", "strategic-thinking", "growth-orientation"] }, blurb: "You see what things could become, and you're patient enough to help them get there. Potential trusts you.",
    picks: [
  ...
  design: { name: "Pattern Illuminator", field: "design",
    sig: { desc: "An early radar for shape and composition, the noticed typeface, the light at the right hour, put to work making things clearer for everyone else. In practice it reads as taste in service of function: the label that finally makes sense, the room that suddenly works, the sentence trimmed to its true weight. Its strength is generosity disguised as aesthetics; clarity is a gift you keep giving. It emerges wherever something is almost right and you can feel the half-degree of correction it needs. Its counterweight is letting rough things stay rough when warmth matters more than polish.", ev: ["Visual-instinct answers", "Comfort & environment choices", "Element affinity with light-leaning reads"], tags: ["aesthetic-judgment", "pattern-recognition", "communication", "craft", "empathy", "attention-to-detail"] }, blurb: "You notice the half-degree that makes things right. Beauty and clarity, in your hands, are the same act.",
    picks: [
  ...
  data: { name: "Insight Navigator", field: "data science",
    sig: { desc: "A pull toward the signal beneath the noise, and a habit of steering by it once found. It tends to make your hunches unusually checkable; you quietly gather evidence before calling a thing true, and you notice when a story and its numbers disagree. Its strength is honest sight, seeing what is actually there, including when that is inconvenient. It emerges when a messy question needs a real answer, or when a pattern flickers where others see coincidence. Its balance is granting some moments immunity from measurement; awe, mostly, works better unquantified.", ev: ["Pattern & panorama answers", "Morning-ritual precision reads", "Multi-question consistency of votes"], tags: ["analytical-thinking", "pattern-recognition", "curiosity", "evidence-based-reasoning", "systems-thinking", "clarity"] }, blurb: "You hear the signal under the noise and follow it honestly. Your hunches come with receipts.",
    picks: [
  ...
  medicine: { name: "Steady Beacon", field: "medicine",
    sig: { desc: "A pattern of tending: noticing what someone needs, often before they say it, and offering it without ceremony. In daily life it reads as warmth people can rest on, the checked-in-on friend, the remembered detail, the cup of tea that arrives unasked. Its strength is trust; people heal a little faster and risk a little more inside your orbit. It emerges wherever someone needs looking after, in ways big and small. Its healthy tradeoff is letting yourself be cared for too, which is why your reading deals you gentle mornings.", ev: ["Care & steadiness answers", "Response-to-pressure reads", "Comfort-food and ritual choices"], tags: ["composure", "empathy", "reliability", "attention-to-detail", "emotional-regulation", "service-orientation"] }, blurb: "You notice what people need before they say it, and you tend to it without ceremony. Being near you feels like rest.",
    picks: [
  ...
  math: { name: "Reflective Architect", field: "mathematics",
    sig: { desc: "A preference for building understanding from first principles and trusting what you have built because you know every beam in it. It can make you allergic to hand-waving and quietly generous with clarity once you have earned it. Its strength is depth: conclusions that hold because they were assembled slowly, in the right order. It emerges when a problem rewards sitting still longer than is fashionable, and in the pleasure of a thing finally proved. Its balance is play, puzzles with no stakes and no audience, which your itinerary quietly supplies.", ev: ["Abstraction & elegance answers", "Optimal-morning structure reads", "Strong-vote conviction on reasoning questions"], tags: ["abstract-reasoning", "rigor", "reflective-thinking", "clarity", "persistence", "intellectual-honesty"] }, blurb: "You build understanding slowly and trust it completely. Depth is your idea of a good time.",
    picks: [
  ...
  product: { name: "Vision Integrator", field: "product",
    sig: { desc: "A habit of holding the big picture and the next concrete step in the same hand: the view chosen, and the route to it. In practice it reads as translation; fog becomes a plan others can act on, and priorities sort themselves under your attention into now, soon, and someday. Its strength is momentum with a direction, since rooms and plans both move when you are in them. It emerges when a room has energy but no shape, or a dream has believers but no first step. Its counterweight is savoring what is finished instead of immediately planning the sequel.", ev: ["Prioritization & instinct answers", "Big-picture view choices", "Cross-question vote synthesis"], tags: ["synthesis", "prioritization", "strategic-thinking", "communication", "decision-making", "momentum"] }, blurb: "You hold the big picture and the next small step in the same hand. Fog turns into footpaths around you.",
    picks: [
  ...
  uxr: { name: "Context Interpreter", field: "UX research",
    sig: { desc: "A practice of understanding things by watching how they are actually used, not how they are described. It tends to make you slow to assume and quick to notice the workaround, the desire path, the thing everyone does but no one mentions. Its strength is accuracy about human beings, a rarer skill than accuracy about facts. It emerges in unfamiliar rooms, new neighborhoods, and any gap between what people say and what they do. Its balance is trusting your own read once the evidence is in, and letting yourself be surprised on purpose now and then.", ev: ["Observation-first answers", "Curiosity & wandering instincts", "Empathy-weighted reads"], tags: ["observation", "empathy", "curiosity", "active-listening", "pattern-recognition", "humility"] }, blurb: "You watch how things are actually done, not how they're described. People make sense to you, which is rarer than it sounds.",
    picks: [
  ...
  teaching: { name: "Resonant Connector", field: "teaching & academia",
    sig: { desc: "A gift for making knowledge land: matching the idea to the person in front of you, sensing when someone is one example away from getting it. Day to day it reads as patience with excellent timing; you do not just know things, you notice what knowing feels like from the other side. Its strength is multiplication, since what you understand tends to end up understood by others. It emerges wherever confusion has made a room quiet. Its tradeoff is protecting time to be the learner yourself, beginner's mind intact.", ev: ["Explanation & patience answers", "Group-instinct reads", "Communication-weighted votes"], tags: ["communication", "empathy", "adaptability", "patience", "mentorship", "clarity"] }, blurb: "You can feel when someone is one example away from understanding. What you know has a way of becoming shared.",
    picks: [
  ...
  fitness: { name: "Kinetic Anchor", field: "fitness & movement",
    sig: { desc: "A body-first intelligence: thinking best in motion, steadying others through consistency of practice. Day to day it reads as follow-through made physical: you show up, and your showing up makes it easier for everyone near you to do the same. Its strength is embodied resilience; moods get metabolized, problems get walked off, confidence gets built one rep at a time. It emerges in movement, effort, and gentle challenge, yours or a friend's. Its tradeoff is genuine rest, treated with the same seriousness as training.", ev: ["Movement & morning answers", "Encouragement instinct reads", "Consistency-weighted votes"], tags: ["discipline", "embodiment", "encouragement", "resilience", "consistency", "vitality"] }, blurb: "You think best in motion and steady others just by showing up. Your consistency is a kind of shelter.",
    picks: [
  ...
  science: { name: "Adaptive Compass", field: "science & research",
    sig: { desc: "A compass set by curiosity, updating course, without drama, as the evidence arrives. It tends to make you comfortable saying 'I don't know, yet,' which quietly makes you one of the more trustworthy people in any room. Its strength is honest wonder: the world stays interesting because you keep letting it correct you. It emerges at the edge of the familiar, new fields, new boroughs, new versions of old beliefs. Its balance is occasionally enjoying settled knowledge without immediately poking it to see if it moves.", ev: ["Inquiry-first answers", "What-if instinct reads", "Willingness-to-update pattern"], tags: ["curiosity", "open-mindedness", "experimentation", "evidence-based-reasoning", "adaptability", "wonder"] }, blurb: "You steer by curiosity and let evidence correct you without drama. The world stays interesting in your hands.",
    picks: [
  ...
  socialwork: { name: "Foundation Builder", field: "social work & community",
    sig: { desc: "An urge to strengthen the ground under other people: noticing who is missing from the room and doing something concrete about it. In practice it reads as care with logistics, the meal train organized, the newcomer folded in, the follow-up that actually happens. Its strength is durable community; things you help build keep standing after the applause moves on. It emerges in friend groups, group chats, stairwells, and stoops. Its counterweight is pouring foundations under your own feet too, which is why your spread includes restorative stops.", ev: ["Community & care answers", "Belonging-instinct reads", "Consistency of other-focused votes"], tags: ["community-building", "empathy", "advocacy", "reliability", "collaboration", "stewardship"] }, blurb: "You notice who's missing from the room and do something about it. The ground is steadier where you've been.",
    picks: [
  ...
  arts: { name: "Creative Prism", field: "film, fashion & the arts",
    sig: { desc: "A lens that refracts the ordinary into something worth looking at twice: a walk becomes a location scout, an outfit becomes a thesis. It tends to make your taste contagious; people see differently after time with you, and they know it. Its strength is transformation on a budget, beauty found rather than bought. It emerges in how you dress a day, a room, a story, a table, yourself. Its balance is unedited experience: some scenes are best left ungraded, unstyled, and gloriously unposted.", ev: ["Aesthetic & imagination answers", "Scene-framing instinct reads", "Era-of-the-city choices"], tags: ["creativity", "aesthetic-judgment", "imagination", "expression", "curation", "playfulness"] }, blurb: "You refract the ordinary into something worth a second look. Life, around you, is better dressed.",
    picks: [
  ...
  law: { name: "Deliberate Pathfinder", field: "law",
    sig: { desc: "A way of moving through complexity carefully without losing momentum, reading the fine print, the room, and the history before choosing a path. In daily life it looks like fairness with a spine: you genuinely hear every side, and then you actually decide. Its strength is trustworthy judgment; people accept outcomes from you that they would contest from anyone else. It emerges when rules, stakes, and people tangle. Its counterweight is joyful improvisation when nothing at all is on the line.", ev: ["Reasoned-argument answers", "Fairness & structure reads", "Conviction pattern across dilemmas"], tags: ["critical-thinking", "fairness", "precision", "advocacy", "decision-making", "integrity"] }, blurb: "You hear every side, then truly decide. Fairness with a spine, care with a compass.",
    picks: [
  ...
  hospitality: { name: "Social Catalyst", field: "food & hospitality",
    sig: { desc: "A talent for running the room without raising your voice: you set the temperature, collide the right people at the right moment, and decide, quietly and completely, how the night goes. People call it charm; it is closer to command. Its strength is convening power, since the gatherings you build become the ones everything else gets measured against. It emerges around food, new faces, and any room that needs an owner. Its balance is handing someone else the reins now and then, and letting yourself be the guest of honor.", ev: ["Care & gathering answers", "Comfort-food generosity reads", "Warmth-weighted votes"], tags: ["generosity", "emotional-intelligence", "attention-to-others", "orchestration", "warmth", "service-orientation"] }, blurb: "Rooms rearrange around you: the right people collide, the night finds its shape. You call it hosting; everyone else calls it gravity.",
    picks: [
  ...
  performing: { name: "Dynamic Synthesizer", field: "music & performing arts",
    sig: { desc: "A feel for a moment's energy and the reflex to shape it in real time: the pause before the punchline, the key change, the exit while it is still good. Day to day it reads as timing you cannot quite teach, knowing when a room needs lifting and when it needs to land. Its strength is aliveness; moments become occasions when you are paying attention to them. It emerges in front of people, planned or not. Its tradeoff is the audience-free hour, where nothing is performed and nothing needs to be, which your quieter stops provide.", ev: ["Presence & timing answers", "Live-music instinct reads", "Energy-mapping votes"], tags: ["presence", "timing", "expressiveness", "adaptability", "momentum", "audience-awareness"] }, blurb: "You read a moment's energy and shape it in real time. Timing isn't something you learned; it's something you are.",
    picks: [
  ...
  writing: { name: "Perspective Weaver", field: "content & writing",
    sig: { desc: "An impulse to turn noticing into narrative: catching the detail that carries the day and giving it back as story. It often makes you the keeper of the version of events people actually remember, and the one who can say the difficult thing so it can finally be heard. Its strength is meaning-making; experience becomes durable in your hands. It emerges in the retelling, over dinner, in the group chat, on the page. Its balance is living some chapters without narrating them; a few scenes are for you alone.", ev: ["Language & noticing answers", "Story-shaped memory reads", "Era-of-the-city instincts"], tags: ["storytelling", "observation", "synthesis", "voice", "empathy", "reflective-thinking"] }, blurb: "You catch the detail that carries the day and hand it back as story. Memory keeps you close.",
    picks: [
...
const HOODS = {
  ...
  lowerEastSide: { name: "Lower East Side", borough: "Manhattan", vibe: "Tenements, smoked fish, galleries, and doors that don't say what's behind them.",
    slots: [
      ...
      ["Lunch", "Russ & Daughters Café or Casa Adela", "A sit-down shrine to bagels, a proper schmear, and lox, or home-style pernil at a beloved Puerto Rican institution on Avenue C."],
  ...
  fortGreene: { name: "Fort Greene", borough: "Brooklyn", vibe: "Brownstones, a hilltop park, and Brooklyn's most graceful cultural block.",
    slots: [
      ...
      ["Dinner", "Miss Ada", "Eastern Mediterranean plates; get the hummus with lamb ragu."],
...
const QUESTIONS = [
  ...
  { q: "Comfort food, no judgment:", type: "element", answers: [
    ...
    { text: "A bodega chop cheese, or a bacon egg and cheese salt pepper ketchup", pts: { iron: 2, ember: 1 } },
...
function computeResult(answerLog) {
  ...
  return { primary, secondary, prof, days, bias, spend, sigRank: pSorted, sigVotes: p };
}


const ELEMENT_SIGILS = {
  light: ["design", "arts"], tide: ["socialwork", "science"], ember: ["hospitality", "performing"],
  storm: ["fitness", "performing"], echo: ["performing", "writing"], mist: ["writing", "uxr"],
  iron: ["engineering", "law"], bloom: ["socialwork", "medicine"], shadow: ["uxr", "data"],
  dream: ["arts", "writing"], frost: ["math", "law"], rock: ["fitness", "engineering"],
};

function sigilsFor(result) {
  const seen = new Set();
  const out = [];
  const push = (k, padded) => {
    if (!k || seen.has(k) || !PROFESSIONS[k]) return;
    seen.add(k);
    const v = (result.sigVotes && result.sigVotes[k]) || 0;
    const conf = padded ? 55 + (out.length === 0 ? 5 : 0) : Math.min(93, 58 + v * 7 + (out.length === 0 ? 5 : 0));
    out.push({ key: k, conf });
  };
  (result.sigRank || [result.prof]).forEach(k => push(k, false));
  (ELEMENT_SIGILS[result.primary] || []).forEach(k => push(k, true));
  (ELEMENT_SIGILS[result.secondary] || []).forEach(k => push(k, true));
  for (const k of Object.keys(PROFESSIONS)) { if (out.length >= 3) break; push(k, true); }
...
function resultFromParams() {
  try {
    ...
    if (p && s && c && ELEMENTS[p] && ELEMENTS[s] && PROFESSIONS[c] && p !== s) {
      ...
      return { primary: p, secondary: s, prof: c, days, bias, spend, sigRank: [c], sigVotes: { [c]: 3 } };
    }
  } catch (e) {}
  return null;
}
...
function splitStops(name) {
  // "A or B" and "A + B" always split; "A & B" splits only when both sides stand alone
  // (so "H Mart & Koryo Books" becomes two stops but "Russ & Daughters" stays whole)
  return name.split(/ \+ | or /).flatMap(part => {
    const bits = part.split(" & ");
    if (bits.length === 2 && bits[0].trim().split(" ").length >= 2 && bits[1].trim().split(" ").length >= 2) return bits;
    return [part];
  }).map(x => x.trim()).filter(Boolean);
}

function mapsLink(H, spend, di) {
  const rows = spend === undefined ? H.slots : expandSlots(H, spend, di);
  const pts = rows.flatMap(r => splitStops(r[1])).map(n => n + ", " + H.name + ", New York, NY");
...
function bonusWildMapsLink(PR, P, S, days) {
  const spot = (name, where) => name + ", " + String(where || "New York").split(" & ")[0] + ", New York, NY";
  const pts = PR.picks.map(w => spot(w.name, w.where));
  pts.push(spot(P.wildcard.name, P.wildcard.where));
  pts.push(spot(S.wildcard.name, S.wildcard.where));
  const NH = HOODS[days[0].hood];
  if (NH.wild) pts.push(spot(NH.wild[0], NH.name));
  const e = encodeURIComponent;
  const mid = pts.slice(1, -1).map(e).join("%7C");
  return "https://www.google.com/maps/dir/?api=1&origin=" + e(pts[0]) +
    "&destination=" + e(pts[pts.length - 1]) + (mid ? "&waypoints=" + mid : "");
}
...
function buildDocExport(days, primary, secondary, prof, spend) {
  ...
  html += "<h2>Bonus stops from your leading sigil, " + esc(PR.name) + "</h2><ul>";
  text += "BONUS STOPS FROM YOUR LEADING SIGIL, " + PR.name.toUpperCase() + "\n";
  ...
  const NW = HOODS[days[0].hood];
  if (NW.wild) { html += "<li><b>" + esc(NW.wild[0]) + "</b> (" + esc(NW.name) + "): " + esc(NW.wild[1]) + "</li>"; text += "  \u2022 " + NW.wild[0] + " (" + NW.name + "): " + NW.wild[1] + "\n"; }
  const bwLink = bonusWildMapsLink(PROFESSIONS[prof], ELEMENTS[primary], ELEMENTS[secondary], days);
  html += "</ul><p><a href=\"" + bwLink + "\">Navigate all bonus stops &amp; wildcards in Google Maps</a></p>";
  text += "\nNavigate all bonus stops & wildcards: " + bwLink + "\n";
  return { html, text };
}
...
export default function App() {
  ...
  const [openSigil, setOpenSigil] = useState(-1);
  const [entered, setEntered] = useState(false);
  useEffect(() => {
    if (screen !== "result") { setEntered(false); return; }
  ...
  const answer = (a) => {
    ...
    if (question.type === "element") {
      ...
      setCheer("🔮 The deck stirs. It has begun tracing your sigils.");
    }
  ...
  if (screen === "intro") return (
    <Shell>
      ...
      <p className="q-whimsy mt-7 text-sm" style={{ color: MUTED, maxWidth: "44ch" }}>
        ...
        draw their cards. The deck reads more than elements: it also traces your sigils, the
        patterns written faintly on your palms in how you move through a day. Don't fight it.
      </p>
    </Shell>
  );
  ...
  return (
    <Shell>
      ...
      {/* Sidekick + profession */}
      <div className="mt-4 mx-auto text-left" style={{ maxWidth: 460 }}>
      ...
      <a href="#itinerary" className="q-eyebrow q-focus no-print inline-block mt-6"
        onClick={(ev) => {
          ev.preventDefault();
          const t = document.getElementById("itinerary");
        ...
        Jump to my itinerary ↓
      </a>

      {/* Sigils: behavioral signatures, tarot-styled (collapsible) */}
      <h2 className="q-display text-2xl mt-10">The sigils in your cards</h2>
      <p className="q-whimsy mt-1.5 text-sm" style={{ color: MUTED }}>
        Patterns in how you move through a day, never labels on who you are.
        Tap to unfold; keep what resonates, leave the rest face-down.
      </p>
      <div className="mt-4 flex flex-col gap-2.5">
        {sigilsFor(result).map(({ key, conf }, i) => {
          const SG = PROFESSIONS[key];
          const openS = printMode || openSigil === i;
          return (
            <div key={key} className="q-card">
              <button aria-expanded={openS} aria-controls={"sigil-panel-" + key}
                onClick={() => setOpenSigil(openS ? -1 : i)}
                className="q-answer q-focus w-full text-left px-5 py-4"
                style={{ background: "transparent", border: "none" }}>
                {i === 0 && <Eyebrow>Leading sigil</Eyebrow>}
                <span className="flex items-baseline justify-between gap-3">
                  <span className="q-display text-[15px]">{SG.name}</span>
                  <span style={{ whiteSpace: "nowrap" }}>
                    <span className="q-eyebrow" style={{ fontSize: 9, color: "#7a5f16" }}>{conf}% resonance</span>
                    <span aria-hidden="true" style={{ fontSize: 10, color: MUTED, marginLeft: 8 }}>{openS ? "▲" : "▼"}</span>
                    <span className="sr-only">{openS ? "Collapse" : "Expand"} details for {SG.name}</span>
                  </span>
                </span>
                {i === 0 && <span className="block text-[13px] mt-2 leading-relaxed" style={{ color: "#4a473f", fontWeight: 400 }}>{SG.blurb}</span>}
              </button>
              <div className="acc-outer" style={{ gridTemplateRows: openS ? "1fr" : "0fr", visibility: openS ? "visible" : "hidden" }}>
                <div style={{ overflow: "hidden", minHeight: 0 }}>
                <div id={"sigil-panel-" + key} role="region" aria-label={SG.name + " details"} className="acc-panel px-5 pb-4" style={{ opacity: openS ? 1 : 0, transform: openS ? "none" : "translateY(-6px)" }}>
                  <p className="text-[13px] leading-relaxed" style={{ color: "#4a473f" }}>
                    {SG.sig.desc}
                  </p>
                  <p className="text-[11px] mt-2" style={{ color: MUTED }}>
                    Traced from: {SG.sig.ev.join(" · ")} · affinity with {P.name} & {S.name}
                  </p>
                  <p className="text-[11px] mt-1.5" style={{ color: MUTED, lineHeight: 1.7 }}>
                    {SG.sig.tags.map(t => "#" + t).join("  ")}
                  </p>
                </div>
                </div>
              </div>
      ...
      {/* Itinerary */}
      <div id="itinerary" className="mt-10 flex items-baseline justify-between" style={{ scrollMarginTop: 16 }}>
        <h2 className="q-display text-2xl">Your six days</h2>
        <Eyebrow>One neighborhood per day</Eyebrow>
      </div>
      <p className="q-whimsy mt-1.5 text-sm" style={{ color: MUTED }}>
        Everything within each day sits in easy proximity, a few gentle blocks or a short ride apart. Tap a stop to open it.
        {bias ? " The deck read your sigils and kept you mostly on the island, candlelight, cobblestones, old money blocks." : ""}
      </p>
      ...
      <div className="mt-5 relative">
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className={"relative pb-3" + (entered ? "" : " anim-up")} style={{ paddingLeft: 60, animationDelay: entered ? undefined : (450 + i * 70) + "ms" }}>
              <div className="absolute" style={{ left: 0, top: 4 }}><Bullet el={d.from} size={44} /></div>
              <button aria-expanded={open} aria-controls={"day-panel-" + i} onClick={() => setOpenDay(open ? -1 : i)}
                className="q-card q-answer q-focus w-full text-left px-5 py-4"
                style={open ? { borderWidth: 2, borderColor: TEXT_SAFE[d.from], background: "#fff", boxShadow: "inset 0 0 0 3px #fff, inset 0 0 0 4px " + TEXT_SAFE[d.from] + "AA" } : undefined}>
                ...
                <p className="text-xs mt-2 font-medium" style={{ color: TEXT_SAFE[d.from] }}>
                  {el.hero.name.split(" the ")[0]} picked this one · <span aria-hidden="true">{open ? "close" : "open"}</span><span className="sr-only">{open ? "Collapse" : "Expand"} Day {i + 1}, {H.name}</span>
                </p>
              </button>
              <div className="acc-outer" style={{ gridTemplateRows: open ? "1fr" : "0fr", visibility: open ? "visible" : "hidden" }}>
                <div style={{ overflow: "hidden", minHeight: 0 }}>
                <div id={"day-panel-" + i} role="region" aria-label={"Day " + (i + 1) + ", " + H.name + " itinerary"} className="acc-panel mt-1.5 mb-0.5 px-5 py-4 flex flex-col gap-3.5" style={{ background: "#fff", border: "1.5px solid " + TEXT_SAFE[d.from] + "66", borderRadius: 16, opacity: open ? 1 : 0, transform: open ? "none" : "translateY(-6px)" }}>
                  ...
                  <a className="q-eyebrow no-print mt-1" href={mapsLink(H, result.spend, i)} target="_blank" rel="noopener noreferrer"
                    style={{ color: INK, textDecoration: "underline", textUnderlineOffset: 3 }}>
      ...
      {/* Bonus stops, dealt by the leading sigil */}
      <h2 className="q-display text-2xl mt-10">Bonus stops from your leading sigil</h2>
      <p className="q-whimsy mt-1.5 text-sm" style={{ color: MUTED }}>{PR.name} deals a few extra stops, slot them into any afternoon.</p>
      <div className="mt-4 flex flex-col gap-2.5">
        {PR.picks.map((w, i) => (
      ...
      <a className="q-eyebrow inline-block mt-3" href={bonusWildMapsLink(PR, P, S, days)} target="_blank" rel="noopener noreferrer"
        style={{ color: INK, textDecoration: "underline", textUnderlineOffset: 3 }}>
        Navigate all bonus stops & wildcards in Google Maps ↗
      </a>

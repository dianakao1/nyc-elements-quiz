...
".deck { position: relative; display: block; width: 234px; height: 314px; margin: 0 auto; background: none; border: none; padding: 0; cursor: pointer; }",
...
function TarotDeck({ idx, onChange }) {
...
const QUESTIONS = [
  ...
  { q: "Pick a scent to bottle and keep.", type: "element", answers: [
    ...
    { text: "Halal-cart smoke at one in the morning", pts: { ember: 2, shadow: 1 } },
    { text: "Night jasmine on a stoop you can't find again", pts: { dream: 2, mist: 1 } },
  ]},
...
function saveResult(r) {
  try { window.localStorage.setItem("nyq-result", JSON.stringify({ p: r.primary, s: r.secondary, c: r.prof })); } catch (e) {}
}
function loadStoredResult() {
  try {
    ...
    if (o && ELEMENTS[o.p] && ELEMENTS[o.s] && PROFESSIONS[o.c] && o.p !== o.s) {
      ...
      return { primary: o.p, secondary: o.s, prof: o.c, days, bias };
    }
  } catch (e) {}
  return null;
}
...
function putResultInURL(r) {
  try { window.history.replaceState({}, "", window.location.pathname + "?p=" + r.primary + "&s=" + r.secondary + "&c=" + r.prof); } catch (e) {}
}
...
function buildDocExport(days, primary, secondary, prof) {
...
async function buildShareCardDataURL(format, primary, secondary, prof) {
  ...
  const arcTxt = ARCANA[primary].title.toUpperCase() + " \u00B7 " + ARCANA[secondary].title.toUpperCase();
  ...
  if (format === "story") {
    ...
    draw(pi, 430, 900, 460, -5); draw(si, 730, 980, 350, 7);
    ctx.fillStyle = CREAM;
    ctx.font = "700 " + fitFont(ctx, names, 62, SG, 940) + "px " + SG; ctx.fillText(names, W / 2, 1420);
    ctx.fillStyle = GOLD; ctx.font = "700 " + fitFont(ctx, arcTxt, 30, SG, 940) + "px " + SG; ctx.fillText(arcTxt, W / 2, 1482);
    ctx.fillStyle = GOLD; ctx.font = "700 24px " + SG; ctx.fillText(lineTxt, W / 2, 1538);
    ctx.fillStyle = CREAM; ctx.font = "italic 500 " + fitFont(ctx, tag, 36, FR, 920, "italic") + "px " + FR; ctx.fillText(tag, W / 2, 1612);
    ...
    draw(pi, 410, 420, 330, -4); draw(si, 680, 455, 260, 6);
    ctx.fillStyle = CREAM;
    ctx.font = "700 " + fitFont(ctx, names, 46, SG, 940) + "px " + SG; ctx.fillText(names, W / 2, 778);
    ctx.fillStyle = GOLD; ctx.font = "700 " + fitFont(ctx, arcTxt, 22, SG, 940) + "px " + SG; ctx.fillText(arcTxt, W / 2, 826);
    ctx.fillStyle = GOLD; ctx.font = "700 19px " + SG; ctx.fillText(lineTxt, W / 2, 868);
    ctx.fillStyle = CREAM; ctx.font = "italic 500 " + fitFont(ctx, tag, 28, FR, 900, "italic") + "px " + FR; ctx.fillText(tag, W / 2, 922);
...
function Bullet({ el, size = 44 }) {
  const d = ELEMENTS[el];
  return (
    <span aria-hidden="true" className="q-display inline-flex items-center justify-center rounded-full flex-shrink-0 leading-none select-none"
...
export default function App() {
  ...
  if (screen === "intro") return (
    <Shell>
      <div className="anim-up" style={{ borderTop: "6px solid " + INK, paddingTop: 18 }}>
        <Eyebrow>New York City · Five-day service · Not too touristy</Eyebrow>
      ...
      <TarotDeck idx={deckIdx} onChange={setDeckIdx} />
  ...
  const copyForDocs = () => {
    const { html, text } = buildDocExport(days, primary, secondary, prof);
        ...
        <div className="no-print" role="dialog" aria-modal="true"
          ...
          <div className="q-card anim-pop" onClick={e => e.stopPropagation()}
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5">
              ...
              <button autoFocus onClick={() => { const mode = preview; setPreview(null); if (mode === "csv") downloadCSV(days); else if (mode === "pdf") printItinerary(); else copyForDocs(); }}
